...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.bcsd.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
    return this.node.services.bcsd.getMiningInfo(function(err, result) {
...
StatusController.prototype.getStakingInfo = function(callback) {
    return this.node.services.bcsd.getStakingInfo(function(err, result) {
...
StatusController.prototype.getDgpInfo = function(req, res) {
  ...
  return this.node.services.bcsd.getDgpInfo(function(err, result) {
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.bcsd.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.bcsd.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.bcsd.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.bcsd.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.bcsd.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.bcsd.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.bcsd.height,
        error: null,
        type: 'bcscore node'
      };
